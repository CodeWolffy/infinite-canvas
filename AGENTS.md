...
- 前端服务请求统一放在 `web/src/services/api/`；本企业分支的图片、文本、画布和素材通过项目 `api/` 后端处理，上游渠道密钥不得下发浏览器。
...
- 本企业分支的画布、素材、生成记录和文本消息以 PostgreSQL、MinIO 为权威存储；浏览器缓存和异步回调必须按登录会话隔离。文件/剪贴板读取、解压、图片处理等异步准备开始时就记录会话版本，在准备完成、发请求和写回状态前核对；不要只在请求函数内部才记录版本。
