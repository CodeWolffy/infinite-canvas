...
+ [修复] 修复本地 Docker 或反向代理映射非标准端口时 Host 端口丢失导致登录报 403「请求来源不受信任」的问题。
+ [新增] 同步上游开源 v0.19.0 版本：新增 ZCode 插件支持，可连接 Canvas Agent MCP 服务读取与操作画布。
+ [优化] 画布图片、元素列表、节点参考栏、资产卡片和生图历史全面改用 WebP 本地缩略图渲染，避免反复解码原图，原图仅在查看原图、下载、导出和生成参考时使用。
+ [优化] 图片和文本节点上层操作按钮改为鼠标移入后显示，避免遮挡画布内容。
+ [优化] 画布多节点拖拽性能优化，组投放检测与位置更新合并至同一帧计算，节点连线支持视口安全剔除，提升复杂画布操作流畅度。
+ [优化] 优化画布保存容错与连接池弹性伸缩：放宽画布引用物理缺失媒体导致的硬拦截并自动复活待删除媒体，数据库连接池上限根据并发配置动态扩容，避免批量生图时数据库连接耗尽与画布保存误报。
+ [修复] 修复画布在放大或查看展开批量卡片时节点被视口虚拟化误裁导致画面消失的问题，完善真实包围盒计算与缩放安全防护。
+ [新增] 支持单个渠道的多上游模型绑定到同一平台模型，并提供全选与批量绑定能力。
+ [优化] 渠道管理支持安全软删除并自动解除模型绑定，画布生成配置面板支持清晰展示模型单张价格。
+ [新增] 适配 OpenAI GPT Image 2.5 系列模型（Flare 与 Sunburst），生图质量支持两档更高思考强度（极高/Very High 与 最大/Max），并更新默认渠道预设与中转别名兼容。
+ [修复] 修复画布生成失败时具体错误被硬编码覆盖为“全部图片生成失败”的问题，优化错误提示的对比度、结构化呈现与内容安全合规拦截识别。
+ [新增] 画布文本生成支持实时打字流式上屏展示，并支持断线自动恢复。
+ [优化] 文本模型根据上游配置动态联动推理思考档位（低/中/高/关闭），隐藏不支持的模型思考开关。
+ [优化] 素材库与表单操作增加提交锁与会话隔离机制，防止重复提交与跨会话数据写入。
+ [优化] 提取统一素材保存能力，打通灵感库、生图工作台与视频任务一键沉淀至资产中心。
+ [新增] 管理后台模型与渠道支持按模态（图片/文本）分 Tab 独立管理，渠道绑定自动按模态过滤防止跨类型错绑。
+ [优化] 渠道连通性探测支持提取完整上游错误原因并对 API 密钥自动掩码脱敏。
+ [新增] 顶栏上线轻量化气泡通知中心，集成全站公告与系统通知双 Tab 切换，消除轮询与弹窗打断。
+ [优化] 模型选择器价格展示支持智能有效小数位格式化与防溢出排版。
