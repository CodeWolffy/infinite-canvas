...
export default function AdminModelsPage() {
    ...
    const bindingMutation = useMutation({ mutationFn: ({ channelId, ...values }: BindingValues) => saveModelChannelBinding(bindingModel!.id, channelId, { ...values, id: editingBindingId || undefined }), onSuccess: () => { void refreshBindings(); setBindingOpen(false); bindingForm.resetFields(); message.success("渠道绑定已保存"); }, onError: notifyError(message.error) });
    const unbindMutation = useMutation({ mutationFn: (bindingId: string) => deleteModelChannelBinding(bindingModel!.id, bindingId), onSuccess: () => { void refreshBindings(); message.success("渠道绑定已移除"); }, onError: notifyError(message.error) });
