...
function session() {
    ...
    return { state, useUserStore: { getState: () => state }, assertCurrentSession: (version) => { if (version !== state.sessionVersion) throw new DOMException("changed session", "AbortError"); } };
}
...
test("a stale generation media upload cannot clear a replacement login", async () => {
    ...
    const generation = load(read("services/api/generation.ts"), context, { "@/services/api/request": request, "@/services/api/media": media });
